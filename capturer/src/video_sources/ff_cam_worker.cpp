...
FFCamWorker::FFCamWorker(QObject *parent)
    : QObject(parent)
{
...
QString FFCamWorker::formatString(const QAudioFormat &format)
{
...
AVSampleFormat FFCamWorker::qAudioFormatToAV(const int &depth, const QAudioFormat::SampleType &sample_format)
{
...
void FFCamWorker::setVideoDevice(Cam::Dev video_device)
{
    this->video_device=video_device;
}

void FFCamWorker::setAudioDevice(int index)
{
    index_device_audio=index;
}

void FFCamWorker::setConfig(QSize size, AVRational framerate, int64_t pixel_format)
{
    cfg.size=size;
...
void FFCamWorker::subscribe(FrameBuffer<Frame::ptr>::ptr obj)
{
    if(!subscription_list.contains(obj))
        subscription_list.append(obj);
}

void FFCamWorker::unsubscribe(FrameBuffer<Frame::ptr>::ptr obj)
{
    subscription_list.removeAll(obj);
}

bool FFCamWorker::isActive()
{
    if(d->format_context)
        return true;
...
AVRational FFCamWorker::currentFrameRate() const
{
    return cfg.framerate;
}

PixelFormat FFCamWorker::pixelFormat() const
{
    if(cfg.pixel_format==PixelFormat::mjpeg)
